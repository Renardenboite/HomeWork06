...
namespace SnakeGame
{
	...
	void Snake::Init()
	{
		...
		addSprite(textures[(size_t)SnakePart::Head], { (float)SCREEN_WIDTH / 2.f, (float)SCREEN_HEIGHT / 2.f });
		addSprite(textures[(size_t)SnakePart::Tail], { (float)SCREEN_WIDTH / 2.f, (float)SCREEN_HEIGHT / 2.f + SNAKE_SIZE * (INITIAL_SNAKE_SIZE - 1) });
	...
	void Snake::UpdateTailSprite()
	{
		...
		if (nextTail->getPosition().x < tail->getPosition().x)
		{
			angle = -90.f;
		}
		else if (nextTail->getPosition().x > tail->getPosition().x)
		{
			angle = 90.f;
		}
	}
